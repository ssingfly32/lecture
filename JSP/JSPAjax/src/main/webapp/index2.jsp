...
<script>
	...
	function outputEmployees(json) {
		...
		$.each(json.employees, function(i, e) {	// i는 인덱스 e는 배열의 i번째의 값을 넣어줌
			...
			output += `<td><img src='./image/modify.png' class='icon' onclick='modEmp(\${e.employee_id});'/></td>`;
	...
	// 사원 수정 버튼 클릭 시
	function modEmp(empNo) {
		
		$('#mFirstName').val();
		$('#mLastName').val();
		$('#mEmail').val().toUpperCase();
		$('#mPhoneNumber').val();
		$('#mHiredate').val();
		$('#mJobId').val();
		$('#mSalary').val();
...
<body>
		...
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="modal-header">
					...
					<button type="button" class="btn-close delEmpModalClose"
	...
	<!-- The 수정 Modal -->
	<div class="modal inputEmpModal" id="modEmpModal">
		<div class="modal-dialog">
			<div class="modal-content">
